import {useState, Fragment} from 'react'


export const ItemCount = ({initial, stock, onAdd})=>{
    const [contador, setContador] = useState(initial)
    const [sumaStock, setSumaStock] = useState(stock)

    const btnSuma = () => {
        if (sumaStock> 1 && contador<sumaStock){
            setContador ( contador + 1)
        }
    }

    const btnResta = () => {
        if (contador>1){
            setContador (contador - 1)
        }
        else if (contador<= 1 ){
            setContador (contador + 0)
        }
    }
    return(
        <Fragment>
        <section className='AgregarCarrito'>
            <div className='contadorStock'>
                <button className='btnContadorMenos' onClick={btnResta}>-</button> 
                <p className='numeroContador' >{contador}</p> 
                <button className='btnContadorMas' onClick={btnSuma}>+</button>
            </div>
            <br></br>
            <div className='btnAgregarCarrito'> 
                <button className='btnSumarCarrito' onClick={()=>onAdd(contador)} disabled={sumaStock === 0 ? true : false } >
                    Agregar al carrito
                </button>
            </div>
        </section>
        </Fragment>
    )
}